...
export default class IsoPhysics {
    ...
    distanceToPointer(displayObjectBody, pointer) {
        ...
        var isoPointer = this.projector.unproject(
            pointer.position,
            undefined,
            displayObjectBody.z
        );
        isoPointer.z = displayObjectBody.z;
        var a = this.anglesToXYZ(
            displayObjectBody,
            isoPointer.x,
            isoPointer.y,
            isoPointer.z
    ...
    anglesToXYZ(displayObjectBody, x, y, z) {
        ...
        var theta = Math.atan2(
            y - displayObjectBody.y,
            x - displayObjectBody.x
        );
    ...
    accelerateToXYZ(
        ...
        displayObject.body.maxVelocity.setTo(
            xSpeedMax,
            ySpeedMax,
            zSpeedMax
        );
    ...
    moveToXYZ(displayObject, x, y, z, speed, maxTime) {
        ...
        if (maxTime > 0) {
            ...
            speed =
                this.distanceToXYZ(displayObject.body, x, y, z) /
                (maxTime / 1000);
        }
        var a = this.anglesToXYZ(displayObject.body, x, y, z);
        var v = this.velocityFromAngles(a.theta, a.phi, speed);
    ...
    moveToPointer(displayObject, speed, pointer, maxTime) {
        ...
        var isoPointer = this.game.iso.unproject(
            pointer.position,
            undefined,
            displayObject.body.z
        );
